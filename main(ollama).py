...
def upload_pdf(mrn: str, file: UploadFile = File(...)):
    """Upload PDF → Extract → Chunk → Store in AstraDB"""
...
async def query_api(request: QueryRequest):
    ...
    context = result.get("context", result.get("answer", ""))
...
# ==============================
# 1️⃣2️⃣ SIMPLE ASK ENDPOINT (Streaming)
# ==============================
...
async def ask_question(request: dict):
    ...
    if not question or not mrn:
        raise HTTPException(status_code=400, detail="Missing question or MRN")
